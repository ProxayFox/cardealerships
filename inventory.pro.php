<?php
	...
	if (array_key_exists("user", $_SESSION)) {
		...
		$result = myDB::getInstance()->getAllInventory();
?>
...
<section class="container border" style="margin-top: 20px; margin-bottom: 75px; background-color: #D6D7D3">
	<h1 style="text-align: center;">These are the cars that are currently in stock</h1>
	<?php 
				...
				if (isset($_GET['update'])) {
					if ($_GET['update'] == 'fail') {
						echo '
						<div class="alert alert-warning" role="alert" style="text-align: center;">
							Something went wrong<br> Or the car is already saved<br>  Please try again.
						</div>';
					}
				}
				if (isset($_GET['update'])) {
			...
			<?php
				if ($result != FALSE) {
					while ($row = $result->fetch_row()) {
					 echo '<tr><td>'.$row[0].'</td> <td><img src="./img/imges/'.$row[1].'" style="width: 100px;"></td> <td>'.$row[2].'</td> <td>$'.$row[3].'</td> <td>'.$row[4].'</td> <td> '.$row[5].'</td> <td>'.$row[6].'</td> <td>'.$row[7].'</td> <td>'.$row[8].'</td> <td>'.$row[9].'</td> <td>'.$row[10].'</td> <td>'.$row[11].'</td> <td>'.$row[12].'</td> <td>'.$row[13].'</td>';
					 ?>
					 	<td>
					 		<form action="mydb/savecar.db.php" method="POST" role="form" data-toggle="validator">
								<div class="col-auto my-1">
									<input name="carID" value="<?php echo $row[0]; ?>" type="hidden">
									<button class="btn btn-success" type="submit">
										Save
									</button>
								</div>
